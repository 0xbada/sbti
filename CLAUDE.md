...
Kakao Developers 콘솔은 2025~2026년 개편으로 도메인 설정이 **두 경로로 분리**됐다. 공유 기능을 완전히 쓰려면 **양쪽 모두** 등록해야 한다.

| 경로 | 역할 | 미등록 시 증상 |
|---|---|---|
| [앱] > [플랫폼 키] > [JavaScript 키] > **JavaScript SDK 도메인** | `Kakao.init()` / API 호출 origin 허용 | SDK init 실패, 공유 버튼 자체 동작 안 함 |
| [앱] > **[제품 링크 관리]** > **웹 도메인** | 공유 메시지 내 `content.link`·`buttons[].link` 수신 도메인 허용 | 이미지·제목·설명은 렌더되는데 **buttons[] 사라짐 + 카드 click-through 무효**. 에러·경고 없이 silent strip. |

증상이 "공유는 성공하는데 일부 Feed 요소만 빠짐" 이면 **코드 디버깅 전에 먼저 [제품 링크 관리] 등록 여부 확인**. 기존 한국어 블로그·튜토리얼은 대부분 개편 전(단일 [플랫폼] > [사이트 도메인] 경로) 기준이라 이 함정을 잡기 어렵다.
